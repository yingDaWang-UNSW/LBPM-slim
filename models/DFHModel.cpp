...
void ScaLBL_DFHModel::Run(){
	...
	while (timestep < timestepMax ) {
		...
		MPI_Barrier(comm);
		//PROFILE_STOP("Update");
//		bool steadyFlag=false;//ydw modification, patch for compatibility with morpho-lbm
//	    analysis.run( timestep, *Averages, Phi, Pressure, Velocity, fq, Den, steadyFlag );
//		if (timestep%1000 == 0) {
//            double volB = Averages->Volume_w();
//            double volA = Averages->Volume_n();
//            double current_saturation = volB/(volA+volB);
//		    if (rank == 0) printf("DFH-LBM -- Time: %d , Sat: %0.4f\n", timestep, current_saturation); 
//        }
	    // Run the analysis
